...
ExecStatus AttentionGenExecutionGPU(Device_Ptr device,
  ...
  if (config.use_hbf && config.hbf_config.num_flash_stacks > 0) {
    ...
    accumul_memory_duration = getAttentionMemoryDuration(config, total_kv_read_size, total_act_size, layer_info.use_chunked_attention, layer_info.chunk_size, 2, layer_info.kv_hbm_fraction, layer_info.kv_write_bytes_flash / 2.0, layer_info.kv_write_bytes_hbm / 2.0);
  } else if (cpuKvOffloadActive(config)) {
  ...
  if (config.use_hbf && config.hbf_config.num_flash_stacks > 0) {
    ...
    accumul_memory_duration = getAttentionMemoryDuration(config, total_context_kv_read_size, total_context_act_size, layer_info.use_chunked_attention, layer_info.chunk_size, 2, layer_info.kv_hbm_fraction, layer_info.kv_write_bytes_flash / 2.0, layer_info.kv_write_bytes_hbm / 2.0);
  } else if (cpuKvOffloadActive(config)) {
